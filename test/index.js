...
test('should process on "md"', () => {
  ...
  assert.strictEqual(output, '<p><strong>Hello World!</strong></p>')
})

test('should return false when not found', () => {
  const transform = inputFormatToTransformer('no-input-format-found')
  assert.strictEqual(transform, false)
})

test('should retrieve the dictionary', () => {
  assert.strictEqual(inputFormatToTransformer.dictionary.babel[0], 'jstransformer-babel')
})
